...
export default function FormEditPlot({id}: any) {
  return (
...
const Form1 = ({id}: any) => {
  ...
  const {plot, editPlot}:any = useContext(companieContext)
  const oldPlot = plot.find((elem: { id: any; })=> {return elem.id === Number(id)})
  const { name } = oldPlot;
  const oldplotAlter = { name };
  const [valueData, setValueData] = useState(oldplotAlter)
  ...
  const onFormSubmit = () => {
    editPlot(valueData, Number(id))
  }
  return (
    <Box bg={'white'} p={'5'} borderRadius={'5%'}>
      <Heading fontSize={"2xl"} mb={'5%'}>Dados do Talhão</Heading>
